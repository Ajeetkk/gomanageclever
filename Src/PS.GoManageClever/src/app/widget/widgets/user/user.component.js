...
class UserComponent extends React.Component {
                    ...
                    x.modules.forEach(y => {
                        y.roles.forEach(z => {
                            if (z.name !== "Service Engineer") {
                                ...
                                this.LstFilterData.Role.push({ "id": z.id, "name": z.name, "code": z.code });
                            }
                        });
                    });
                }
    ...
    SubmitPopup = (actionType, data) => {
        switch (actionType) {
            ...
            case DefaultStrings.Role:
                let activeTabName1 = this.tabs.filter(x => x.IsActive);
                if (activeTabName1[0].Name === 'Active') {
                    const testData = { roles: data };
                    this.userService.updateRoles(testData, this.state.PopupData.id).then(data => {
                        this.ClosePopup();
                        if (data.status === DefaultStrings.SucessCode) {
                            this.loadUsers(this.state.Page);
                        }
                    ...
                    this.userService.updateInviteUserRoles(roleData, this.state.PopupData.userInvitationId).then(data => {
                        this.ClosePopup();
                        if (data.status === DefaultStrings.SucessCode) {
                            this.loadInviteUsers(this.state.Page);
                        }
    ...
    prepareActiveHeader() {
        ...
        lstHead.push({ "Text": "Email", "IsDefault": false, "IsSort": true, "Type": "Text", "SortText": "alternateEmail", "ClassName": "sort-order" });
        lstHead.push({ "Text": "Phone", "IsDefault": false, "IsSort": true, "Type": "Text", "SortText": "telephonenumber", "ClassName": "sort-order" });
    ...
    prepreActiveBody(userData) {
        let lstBody = [];
        userData.forEach(e => {
            lstBody.push({ "body": [{ "Text": "", "Type": "CheckBox" }, { "Text": e.displayName, "Type": "Label" }, { "Text": e.alternateEmail, "Type": "Email" }, { "Text": e.countryCode + ' '+ e.telephoneNumber, "Type": "Telephone" }, { "Text": { "Text": this.prepeareUserView(e.modules), "Object": e }, "Type": "ActionClick" }, { "Text": { "Action": ["deleteIcon"], "Object": e }, "Type": "Icon" }] })
        });
        return lstBody;
    }
    ...
    prepareInviteHeader() {
        ...
        lstHead.push({ "Text": "Manage role", "IsDefault": false, "IsSort": false, "Type": "Text", "SortText": "Email", "ClassName": "sort-order" });
        lstHead.push({ "Text": "Action", "IsDefault": false, "IsSort": false, "Type": "Text", "ClassName": '' });
        return lstHead;
    }
    ...
    prepareInviteBody(userData) {
        let lstBody = [];
        userData.forEach(e => {
            lstBody.push({ "body": [{ "Text": e.email, "Type": "Email" }, { "Text": { "Text": this.prepeareUserView(e.modules), "Object": e }, "Type": "ActionClick" }, { "Text": { "Action": ["reloadIcon", "revokeIcon"], "Object": e }, "Type": "Icon" }] })
        });
        return lstBody;
    }
        ...
        return (
            <div>
                <UserView header={this.state.Header} body={this.state.Body}
                    ...
                    FilterData={this.LstFilterData} IsClickable={false} SelectedRow={this.SelectedRow}
