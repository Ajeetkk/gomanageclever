...
export default class EngineService {
    ...
    getUnassignEnginList(config) {
        return this.baseApiService.get(`${this.baseUrl}devices?pagesize=${config.pagesize}&pageindex=${config.pageindex}&active=${true}&sort=${encodeURIComponent(config.sort)}${config.search ? `&search=${encodeURIComponent(config.search)}` : ''}&isactive=${true}&isassigned=${false}&include=All`, '', false);
    }
    unAssignEngine(data) {
