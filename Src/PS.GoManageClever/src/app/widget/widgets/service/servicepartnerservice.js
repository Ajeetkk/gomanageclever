...
export default class ServicePartnerService {
  ...
  getAssignmentServicePartners(config, url) {
    ...
    return this.baseApiService.get(
      url +
      "companies/1446/assets?pagesize=" +
      config.pagesize +
      "&pageindex=" +
      config.pageindex +
      "&isactive=true" +
  ...
  assignServicePartner(dataforAssign, servicePartnerId) {
    return this.baseApiService.post(this.BaseUrl + 'servicepartners/' + servicePartnerId + '/assets', dataforAssign, true)
  }
